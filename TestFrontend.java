...
public class TestFrontend {
	...
	public void runTests() {
		...
		System.out.print("Test '2' to remove low rated movies(This will always pass if backend is not implemented): ");
	...
	public boolean enterXToExit() {
		...
		try {
			...
			((Frontend)frontend).run(null); //!!!Remove this line when you uncomment the one above
	...
	public boolean testFrontendGForGenres() {
		...
		try {
			...
			String input = "g" + System.lineSeparator() + "x" + System.lineSeparator() + "x" + System.lineSeparator();
			InputStream inputStreamSimulator = new ByteArrayInputStream(input.getBytes());
			System.setIn(inputStreamSimulator);
			ByteArrayOutputStream outputStreamCaptor = new ByteArrayOutputStream();
			// set the output to the stream captor to read the output of the front end
			...
			if (frontend != null && appOutput.contains("Horror")
							...
							/*&& appOutput.contains("Musical")*/) {  //Will be necessary to test when backend is implemented
	 ...
	 * the app by pressing 'x' to go back to the main mode and another 'x' to exit.
	 * The test succeeds if the rating selection screen contains all ten possible ratings
	 * from the data. It fails if any of them are missing, the front end has not
	 * been instantiated (is null), or there is an exception.
	 * @return true if the test passed, false if it failed
	 */
	...
	/**
	 * This test runs the front end and redirects its output to a string. It then
	 * passes in 'r' as a command to go to the rating selection mode, and then passes '2' to deselect movies with a rating between 2 and 3. It then exits
	 * the app by pressing 'x' to go back to the main mode and another 'x' to exit.
	 * It then removes the part of the output string from before passing '2'
	 * The test succeeds if the rating selection screen contains only 1 reference to the movie with a rating of 2.9
	 * It fails if the front end has not
	 * been instantiated (is null), or there is an exception.
	 * @return true if the test passed, false if it failed
	 */
	public boolean testFrontend2ForRatings() {
		...
		try {
			// set the input stream to our input (with an g to test of the program lists genres)
			String input = "r" + System.lineSeparator() + "2" + System.lineSeparator() + "x" + System.lineSeparator() + "1" + System.lineSeparator() + "x" + System.lineSeparator();
			InputStream inputStreamSimulator = new ByteArrayInputStream(input.getBytes());
			System.setIn(inputStreamSimulator);
			ByteArrayOutputStream outputStreamCaptor = new ByteArrayOutputStream();
			// set the output to the stream captor to read the output of the front end
